...
bool compare_calories(int x, int z){

    if(x.calories < z.calories){
      return true;
    }
    else
    {
...
const int *min( const int* first, const int* last, bool (*compare)(auto, auto) )
{
    // Avoid invalid input.
    ...
    {
        // if (cmp ( *first, *S ))
        if ( *compare ) // content of `first` is smaller than content of `s`.
...
int main( void )
{
  ...
  // std::cout << ">>> The smallest element in A is \"" << *result << "\"" << std::endl;
    ...
    auto result2 = min(std::begin(bag), std::end(bag), compare_calories(std::begin(bag), std::end(bag)));
